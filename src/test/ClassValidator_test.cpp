...
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::Class_info(2), ConstantPool::Integer_info(1234)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}
...
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::String_info(2)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestStringInfoWrongUtf8Index)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::String_info(2), ConstantPool::Integer_info(1234)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestStringInfoValid)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::String_info(2), JUtf8String("foo")});
	ASSERT_NO_THROW(ClassValidator::validateConstantPool(cp, 52, 0));
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidReferenceKind)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(static_cast<ConstantPool::reference_kind>(10), 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexGetField)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::getField, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexGetStatic)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::getStatic, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexInvokeInterface)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::invokeInterface, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexInvokeSpecial)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::invokeSpecial, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexInvokeStatic)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::invokeStatic, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexInvokeVirtual)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::invokeVirtual, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexNewInvokeSpecial)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::newInvokeSpecial, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexPutField)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::putField, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}

TEST_F(ClassValidatorTest, TestMethodHandleInfoInvalidIndexPutStatic)
{
  ConstantPool cp(std::vector<ConstantPool::cp_type>{ConstantPool::tag::Invalid, ConstantPool::MethodHandle_info(ConstantPool::reference_kind::putStatic, 1)});
	ASSERT_THROW(ClassValidator::validateConstantPool(cp, 52, 0), std::runtime_error);
}
